...
const fieldClass =
  "mt-1.5 w-full rounded-2xl border border-input bg-background px-4 py-3 text-sm text-foreground outline-none transition-colors placeholder:text-muted-foreground/70 focus:border-primary focus:ring-2 focus:ring-ring/30";
...
export function AppointmentForm() {
  ...
  if (status === "sent") {
    return (
      <div className="rounded-3xl border border-border bg-secondary/60 p-8 text-center">
        <CheckCircle2 className="mx-auto h-10 w-10 text-primary" strokeWidth={1.6} />
  ...
  return (
    <form
      onSubmit={onSubmit}
      noValidate
      className="rounded-3xl border border-border bg-card p-6 shadow-[0_28px_60px_-48px_rgba(31,46,23,0.9)] sm:p-8"
    >
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2">
          <label htmlFor="preferred_date" className="text-sm font-medium text-foreground">
        ...
        <div className="sm:col-span-2">
          <label htmlFor="message" className="text-sm font-medium text-foreground">
            What's troubling you? <span className="text-muted-foreground">(optional)</span>
          </label>
          <textarea
            id="message"
            name="message"
      ...
      <button
        ...
        className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-full bg-primary px-7 py-3.5 text-base font-semibold text-primary-foreground transition-transform hover:-translate-y-0.5 disabled:opacity-70"
      >
